...
public class UsuarioService {
  ...
  private List<Usuario> usuarios = new ArrayList<>(List.of(
          new Usuario(1, "Marcelos Salas", "marcelo.salas@example.com", 32),
          new Usuario(2, "Michael Perez Yackson", "michael.yackson@example.com", 54),
  ...
  public UsuarioDTO getUsuarioById(int id) {
    return usuarios.stream()
            ...
            .map(usuario -> new UsuarioDTO(usuario.getId(), usuario.getNombre()))
            .findFirst()
            .orElse(new UsuarioDTO(-1, "Usuario no encontrado"));
  }
}
